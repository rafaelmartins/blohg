...
class BlohgBlueprintTestCase(unittest.TestCase):
    ...
    def test_jinja_loader_from_disk(self):
        ...
        self.assertEqual(len(bp.jinja_loader.searchpath), 1)
        self.assertEqual(bp.jinja_loader.searchpath[0], fake_dir)
    ...
    def test_jinja_loader_from_repo_package(self, get_root_path):
        ...
        self.assertEqual(bp.jinja_loader.template_folder,
                          'ext/blohg_foo/templ')
    ...
    def test_jinja_loader_from_repo_module(self, get_root_path):
        ...
        bp = BlohgBlueprint('foo', __name__, template_folder='templ')
        self.assertIsInstance(bp.jinja_loader, BlohgLoader)
        self.assertEqual(bp.jinja_loader.template_folder, 'ext/templ')
    ...
    def test_static_folder_from_disk(self):
        ...
        self.assertEqual(view_func.__name__, 'send_static_file')
    ...
    def test_static_folder_from_repo_package(self, get_root_path):
        ...
        self.assertEqual(view_func.directory, 'ext/blohg_foo/st')
    ...
    def test_static_folder_from_repo_module(self, get_root_path):
        ...
        view_func = app.view_functions['foo.static']
        self.assertIsInstance(view_func, BlohgStaticFile)
        self.assertEqual(view_func.directory, 'ext/st')
...
""")
    ...
    def test_import_module(self):
        import blohg_foo
        self.assertEqual(blohg_foo.ext_id, 'foo')
        del sys.modules['blohg_foo']

    def test_import_package(self):
        import blohg_bar
        self.assertEqual(blohg_bar.ext_id, 'bar')
    ...
    def test_import_broken_extension(self):
        try:
            ...
            t, v, tb = sys.exc_info()
            mtb = tb
            while mtb.tb_next is not None:
                mtb = mtb.tb_next
            self.assertEqual(mtb.tb_frame.f_code.co_filename,
                             ':repo:ext/blohg_baz.py')
            self.assertEqual(mtb.tb_frame.f_lineno, 3)
        else:
